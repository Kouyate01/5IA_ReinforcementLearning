...
class PolicyNetwork(nn.Module):
    ...
    Le masquage des actions illégales se fait en dehors du réseau.
    """
...
class REINFORCE:
    """
    Agent REINFORCE — gradient de politique Monte-Carlo (Williams, 1992).

    ALGORITHME
    ──────────
    À chaque épisode :
        1. Suivre la politique π_θ pour collecter la trajectoire (s_t, a_t, r_t)
        2. Calculer les retours Monte-Carlo :
               G_t = Σ_{k≥t} γ^(k-t) · r_k
        3. Mise à jour par gradient ascendant :
               ∇θ J(θ) ≈ Σ_t  ∇θ log π(a_t | s_t ; θ) · G_t

    Sans baseline → variance élevée, convergence lente sur les grands espaces.
    Voir REINFORCEMeanBaseline pour la version avec baseline moyenne.
    ...
    def train(
        ...
        TensorBoard :
            ...
            train/mean_loss   : perte de politique moyenne (−Σ log π · G_t)
    ...
    def _collect_episode(self, env: BaseEnv):
        ...
        Retourne les log-probabilités des actions jouées et les récompenses.
        """
        state     = env.reset()
        log_probs = []
        rewards   = []
    ...
    def _update_policy(self, log_probs: list, returns: list) -> float:
        """
        Mise à jour du réseau de politique par gradient ascendant.
