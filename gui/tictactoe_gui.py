...
class GameTab(tk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self._auto_job = None  # after() id pour Random vs Random
    ...
    def _new_game(self):
        ...
        if mode == "Random vs Random":
            self._status_text.set("Random vs Random — en cours…")
            self._schedule_random_step()
    ...
    def _hvr_random_step(self):
        ...
        self._refresh_display()
        if done:
            # reward est du point de vue de O (player1), donc -reward pour player0
    ...
    def _schedule_random_step(self):
        self._auto_job = self.after(250, self._random_step)
    ...
    def _random_step(self):
        ...
        if not actions:
            return
        action = random.choice(actions)
        player_before = self._env._current_player
        self._last_action = action
        _, reward, done = self._env.step(action)
        self._refresh_display()
        if done:
            self._end_game(reward if player_before == 0 else -reward, from_player=player_before)
        else:
            self._schedule_random_step()
    ...
    def _end_game(self, reward_for_p0: float, from_player: int = 0):
        ...
        if env._winner == 0:
            mode = self._mode.get()
            if mode == "Human vs Human":
                self._status_text.set("🏆 Joueur X gagne !")
            else:
                self._status_text.set("🏆 Vous avez gagné ! (X)")
            self._stats.record("win")
        elif env._winner == 1:
            mode = self._mode.get()
            if mode == "Human vs Human":
                self._status_text.set("🏆 Joueur O gagne !")
            else:
                self._status_text.set("😔 O gagne. Retentez votre chance !")
